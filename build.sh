...
run_guacamole () {
    ...
    podman-compose -f ${BASE_DIR}/containers/dev/compose.yml up
    podman-compose -f ${BASE_DIR}/containers/dev/compose.yml down
    #podman run --rm -d --name "invocado_guacd" --pod invocado guacamole/guacd
    #podman run --rm --name "invocado_db" -e "MYSQL_RANDOM_ROOT_PASSWORD=yes" -e "MYSQL_DATABASE=db" -e "MYSQL_USER=user" -e "MYSQL_PASSWORD=password" -v "${BUILD_DIR}/db/initdb.sql:/initdb.sql:ro" --pod invocado mysql:8.1
